...
function* loadWithoutErrorAnnotation(
  ...
  if (moduleDescriptor === undefined) {
    ...
    if (moduleHook === undefined) {
      ...
      Fail`${moduleHookName} needed to load module ${q(
        moduleSpecifier,
    ...
    if (moduleDescriptor.namespace !== undefined) {
      ...
      if (typeof moduleDescriptor.namespace === 'string') {
        if (moduleDescriptor.compartment !== undefined) {
          const { compartment: aliasCompartment, namespace: aliasSpecifier } =
            moduleDescriptor;
          if (
          ...
          Fail`SES does not implement namespace module descriptors with the default (parent) compartment`;
        }
      }
